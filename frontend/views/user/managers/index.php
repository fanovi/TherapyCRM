...
<div class="mx-auto max-w-full p-4 md:p-6">
    ...
    <div x-data="{ pageName: '<?= Html::encode($this->title) ?>'}">
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            ...
            <div>
                <?= Html::a('<svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> Nuovo Manager', 
                    ['create-manager'], [
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-5 py-4 sm:px-6 sm:py-5">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Gestisci tutti i manager del sistema.
            </p>
        </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <span class="text-sm text-gray-600 dark:text-gray-400">
                        <?= 'Trovati ' . $dataProvider->totalCount . ' manager' ?>
                    </span>
                </div>
        ...
        <div class="border-t border-gray-100 dark:border-gray-800 overflow-x-auto">
            <?php Pjax::begin(['id' => 'manager-grid-pjax']); ?>
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'urlCreator' => function ($action, $model, $key, $index) {
                            if ($action === 'view') {
                                return ['view-manager', 'id' => $model->id];
                            }
                            if ($action === 'update') {
                                return ['update-manager', 'id' => $model->id];
                            }
                            if ($action === 'toggle-status') {
                                return ['toggle-status-manager', 'id' => $model->id];
                            }
                        ...
                        'buttons' => [
                            ...
                            'update' => function ($url, $model, $key) {
                                if (!Yii::$app->user->can('update_manager')) return '';
                                return Html::a(
                                    '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            'toggle-status' => function ($url, $model, $key) {
                                if (!Yii::$app->user->can('delete_manager')) return '';
                                
                                if ($model->status == 'active') {
                                    // Show deactivate button for active managers
                                    return Html::a(
                                        '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                        </svg>', 
                                        $url, [
                                        'title' => 'Disattiva manager',
                                    ...
                                    // Show activate button for inactive managers
                                    return Html::a(
                                        '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>', 
                                        $url, [
                                        'title' => 'Attiva manager',
...
document.addEventListener('DOMContentLoaded', function() {
    const pjaxContainer = '#manager-grid-pjax';
