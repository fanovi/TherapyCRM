...
<div class="mx-auto max-w-4xl p-4 md:p-6 min-h-screen">
    ...
    <!-- Script per validazione lato client migliorata -->
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                const passwordRepeat = form.querySelector('[name="User[password_repeat]"]');
                
                if (password && passwordRepeat) {
                    if (password.value !== passwordRepeat.value) {
                        e.preventDefault();
                        const errorDiv = passwordRepeat.closest('.form-group').querySelector('.help-block');
                        if (errorDiv) {
                            errorDiv.textContent = 'Le password devono coincidere.';
                    ...
                    if (password.value.length < 6) {
                        e.preventDefault();
                        const errorDiv = password.closest('.form-group').querySelector('.help-block');
                        if (errorDiv) {
                            errorDiv.textContent = 'La password deve contenere almeno 6 caratteri.';
                            errorDiv.style.display = 'block';
                        }
                        return false;
                    }
